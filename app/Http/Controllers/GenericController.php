...
class GenericController extends Controller
{
    ...
    public function financialData(Request $request)
    {
        ...
        $formattedExpenses = $expenses->map(function ($expense) use ($expenseController) {
            return $this->formatExpenseForUnified($expense);
        });

        // Format receipts
        $receiptController = new ReceiptController();
        $formattedReceipts = $receipts->map(function ($receipt) use ($receiptController) {
            return $this->formatReceiptForUnified($receipt);
        });

        // Format debts
        $debtController = new DebtController();
        $formattedDebts = $debts->map(function ($debt) use ($debtController) {
